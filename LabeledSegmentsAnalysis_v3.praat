...
form Analysis of labeled Intervals
  comment Analysis files:
    sentence input_directory_data C:\
	sentence input_directory_textgrid C:\
	sentence output_directory C:\
    sentence input_files *.wav
  comment Result file will be output_directory \ (name of .wav data file) _ (tiernumber) _ ...
    sentence result_file results.txt
  comment What is the last part of the of the TextGrid file? (name of .wav data file)+... ? 
	word tgname _aligned
...
smooth_pitch = 0
interpolate_pitch = 0
# determine data_directory:
if (right$(input_directory_data$,1) = "/")
  data_directory$ = input_directory_data$
else
  data_directory$ = input_directory_data$ + "/"
endif

# determine textgrid_directory:
if (right$(input_directory_data$,1) = "/")
  textgrid_directory$ = input_directory_textgrid$
else
  textgrid_directory$ = input_directory_textgrid$ + "/"
endif
...
if (left$(tgextension$, 1) = ".")
   tgextension$ = tgextension$
else
   tgextension$ = "." +tgextension$
endif
...
# printline
# printline Settings:
# printline Input files 'tab$' 'data_directory$''input_files$'
...
#fileappend "'resultFile$'" 'data_directory$''input_files$', Tier number 'tier_number''newline$'
#fileappend "'resultFile$'" Results:'newline$'
##### End writing settings ##################

########### Make list of audio files #############
#Create Strings as file list... list 'data_directory$''input_files$'
Create Strings as file list... list 'textgrid_directory$'*'tgextension$'
...
for ifile to numberOfFiles
   ...
   fileName$ = replace$ (fileName$, ".TextGrid", ".mp3", 0)

   Read from file... 'data_directory$''fileName$'
   #resultFile$ = output_directory$ + result_file$
   resultFile$ = output_directory$+left$(fileName$, length(fileName$)-4)+".txt"
   ...
   Read from file... 'textgrid_directory$''soundname$''tgname$''tgextension$'
   select TextGrid 'soundname$''tgname$'
   ...
   ##### save TextGrid file #####
   #select TextGrid 'soundname$''tgname$'
   #Write to text file... 'textgrid_directory$''soundname$''tgname$''tgextension$'
...
### Write info screen ###
filedelete 'data_directory$'analysis.info
fappendinfo 'data_directory$'analysis.info
  ...
  ##### if INTERVAL tier #####
   if (tierType$ = "interval") or (tierType$ = "Interval")
      ...
      for interval from 2 to (numberOfIntervals-1)
         ...
         if (index>0) or (interval$ = "#") or ((interval$ = "*") and ((label$ <> "") and (label$ <> "<SIL>") and (label$ <> "SIL") and (label$ <> "<SPN>") and (label$ <> "[SPN]")))
